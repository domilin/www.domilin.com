...
// 根据日期生成文件
const transportFile = (obj: anyType): anyType =>
  new (winston.transports as anyType).DailyRotateFile(
    Object.assign(
      {
...
// 信息输出
const formatPrintf = (info: anyType): anyType => {
  try {
    ...
    if (info.meta) {
      ...
      meta.error &&
        Object.keys(meta.error).map(function (key) {
          // 暂时有errpath， errmessage
          obj[`err_${key}`] = isObject(meta.error[key])
            ? JSON.stringify(meta.error[key])
            : meta.error[key];
        });
      ...
      meta.res &&
        Object.keys(meta.res).map(function (key) {
          // 暂时有resstatusCode
          obj[`res_${key}`] = isObject(meta.res[key])
            ? JSON.stringify(meta.res[key])
            : meta.res[key];
        });
...
const transportLevelFile = (level: string): void =>
  transportFile({
    level: level,
